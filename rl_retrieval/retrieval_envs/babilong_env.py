...
class BabilongEnv(RetrievalEnv):
    ...
    def __init__(self,
        ...
        super().__init__(reward_model, None, max_steps)
        ...
        assert (embedder is None) + int(embed_tokenizer is None) != 1, "either embedder or tokenizer is not specified"
    ...
    def reset(self, new_sample=None, seed=None):
    ...
    def step(self, chosen_acts):
        ...
        if ( np.array(self.chosen_sent_ids) >= len(self.sentences) ).any():
    ...
    def _make_state(self):
        ...
        if self.embedder is not None:
            ...
            info['state_embed'] = self.get_embeds(s)[0]
            info['acts_embed'] = self.sent_embeds
