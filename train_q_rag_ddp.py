...
import torch
import sys
from rl.agents.pqn_nnm import PQN
import numpy as np
...
print("Process rank", rank, device_id)
#####
...
agent_ddp = DDP(PQN(agent_config).to(device=device_id), device_ids=[device_id])
agent = agent_ddp.module
#####
# def get_module_dir(obj):
#     return [attr for attr in dir(obj) if isinstance(getattr(obj, attr), (torch.nn.Module, torch.nn.ModuleList, torch.nn.ModuleDict))]

# for mname in get_module_dir(agent):
#     m = getattr(agent, mname)
#     try:
#         setattr(agent, mname,  DDP(m.to(device=device_id), device_ids=[device_id]))
#     except:
#         print("AAAAA")
#####
