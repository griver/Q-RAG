...
Dual-GPU 48GB Optimized version (CPU offloading removed for max speed).
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="GPU id used by vLLM / QA inference and Planner",
    )
    p.add_argument(
        "--train_gpu_id",
        type=int,
        default=1,
        help="GPU id used by Critic training",
    )
    p.add_argument(
...
# ─────────────────────────────────────────────────────────────
#  v2: GRPO loss with CIV advantage
# ─────────────────────────────────────────────────────────────
def grpo_loss_civ(
    samples,
    rewards,
...
def train_one_stage(
    ...
    for sample_idx, data in enumerate(tqdm(stage_data, desc=f"  Stage {stage_id} training")):
        ...
        # ── Step 4: Stratified group sampling ────────────────
        critic_model.train()
        ...
        # ── Step 5: Evaluate each sample, compute dual-objective reward ──
        group_rewards = []
        group_verdicts = []
...
def main():
    ...
    # ── Step 1: vLLM on dedicated QA GPU (GPU 0) ─────────────
    ...
    # ── Load Planner (frozen) — 直接焊死在 GPU 0 上 ───────────
    print(f"\n[2/3] Loading Planner (frozen) on GPU{args.qa_gpu_id} ...")
    ...
    planner_base_model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map={"": args.qa_gpu_id},      # <--- 直接指定到跑 vLLM 的同一张卡上
    ...
    print(f"Planner ready on GPU{args.qa_gpu_id}.")
