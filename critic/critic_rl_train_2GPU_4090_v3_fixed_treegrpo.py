...
OOM Fixes applied:
  - Planner offloaded to CPU when not in use (~14GB freed)
  - Gradient checkpointing enabled on Critic
  - MICRO_BS reduced from 2 to 1
  - Reference forward pass runs before policy forward pass in grpo_loss_civ
...
def parse_args():
    ...
    p.add_argument("--output_dir", type=str, default="./critic_lora_grpo_tree")
...
def compute_batched_response_logprobs(
    ...
    # [OOM FIX #3] MICRO_BS 从 2 改为 1，减少单次前向的显存占用
    MICRO_BS = 1
    all_token_log_probs = []
...
def grpo_loss_civ(
    ...
    # [OOM FIX #4] 先跑 reference（无梯度），释放显存后再跑 policy（有梯度）
    # ── reference log-prob (base model w/o adapter) — 先算，先释放 ──
    with torch.no_grad():
    ...
    ref_avg_log_probs = ref_avg_log_probs.detach().clone()
    torch.cuda.empty_cache()

    # ── policy log-prob (per-token avg) — 需要梯度 ──────────
...
# ─────────────────────────────────────────────────────────────
#  Planner 设备管理辅助函数 [OOM FIX #1]
# ─────────────────────────────────────────────────────────────
def _move_planner(planner_model, target_device):
    """将 Planner 模型搬到指定设备，并释放 GPU 缓存。"""
    planner_model.to(target_device)
    if str(target_device) == "cpu" or target_device == torch.device("cpu"):
        torch.cuda.empty_cache()
...
def train_one_stage(
    ...
    def tree_step(question, feedback, critic_model, critic_tok, group_size, temperature, max_new_tokens, tree_depth=DEFAULT_TREE_DEPTH):
        plan = cached_planner(question, feedback=feedback)
        if tree_step == 0:
            for _ in range(MAX_RETRIES - DEFAULT_TREE_DEPTH):
                final_plan = plan
                v2, feedback = cached_zero_temp_critic(question, final_plan)
                if v2 == "ACCEPT":
                    return final_plan
                else: plan = cached_planner(question, feedback=feedback)
        
        prompt_key = critic_prompt_cache_key(question, plan)
        if prompt_key not in critic_prompt_cache:
            critic_prompt_cache[prompt_key] = build_critic_prompt(
                question, plan, critic_tok,
            )
        ...
        for sample in group_samples:
            ...
            if verdict == "ACCEPT" and not appended:
                all_plan.append(plan)
                appended = True
            else: plan.extend(tree_step(question, feedback, critic_model, critic_tok, group_size, temperature, max_new_tokens, tree_depth=tree_depth - 1))
        return all_plan
    # def cached_agent_loop_tree(question, final_plan, context):
    #     key = agent_loop_cache_key(question, final_plan, context)
    #     if key not in agent_loop_cache:
    #         for item in run_agent_loop_tree(
    #             question, final_plan, context, llm, qa_tok,
    #         ):
    #             agent_loop_cache[key] = 
    #     return agent_loop_cache[key]
    ...
    for sample_idx, data in enumerate(tqdm(stage_data, desc=f"  Stage {stage_id} training")):
        ...
        for sample in group_samples:
            ...
            # TODO tree this whole thing
            else:
                # REJECT → replan, then evaluate
                all_plan = tree_step(question, feedback, critic_model, critic_tok, 2, temperature=args.critic_temp, max_new_tokens=args.max_new_token) # FIXME
                for final_plan in all_plan:
        ...
        if not loss.requires_grad:
            raise RuntimeError(
                "Loss does not require grad. This usually means the critic forward graph was detached. "
                "Check gradient checkpointing, input grads, and any accidental no_grad/inference_mode "
                "around the policy forward pass."
            )
...
def main():
    ...
    # ── Load Planner (frozen) — 初始加载到 CPU [OOM FIX #1] ──
    print(f"\n[2/3] Loading Planner (frozen) — will be moved to GPU on demand ...")
    ...
    planner_base_model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="cpu",                    # [OOM FIX #1] 初始加载到 CPU
    ...
    print(f"Planner ready on CPU (will move to GPU{args.train_gpu_id} on demand).")
    ...
    # [TRAIN FIX #1] gradient checkpointing + LoRA 训练时，必须让输入嵌入保留梯度
    # 否则会出现：None of the inputs have requires_grad=True / loss has no grad_fn
    critic_model.config.use_cache = False
    critic_model.gradient_checkpointing_enable()
    if hasattr(critic_model, "enable_input_require_grads"):
        critic_model.enable_input_require_grads()
    else:
        def _make_inputs_require_grad(module, input, output):
            output.requires_grad_(True)
        critic_model.get_input_embeddings().register_forward_hook(_make_inputs_require_grad)

    critic_model.print_trainable_parameters()
    print(f"Critic ready on GPU{args.train_gpu_id} (gradient checkpointing enabled, input grads enabled).")
