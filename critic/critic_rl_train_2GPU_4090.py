...
Dual-GPU optimized version (ideal for dual RTX 4090):
    - GPU0 runs vLLM / QA inference
    - GPU1 runs Planner + Critic training
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="GPU dedicated to vLLM: set this high enough for QA throughput but leave headroom if needed",
    )
    p.add_argument(
        "--vllm_max_model_len",
        type=int,
...
def main():
    ...
    if args.qa_gpu_id == args.train_gpu_id:
        raise RuntimeError(
            f"qa_gpu_id and train_gpu_id must be different for dual-GPU mode. Got both={args.qa_gpu_id}."
        )
    ...
    print(
        ...
        f"qa_gpu=GPU{args.qa_gpu_id} | train_gpu=GPU{args.train_gpu_id} | qa_max_tokens={QA_MAX_TOKENS} | "
