...
from dataloaders.utils import Task
#Most of this file contains code from https://github.com/booydar/babilong
...
class TaskDataset(Dataset):
    ...
    def __init__(self, data_source_path, task, split, max_n_facts=None):
        super().__init__()
        self.task = task
        self.split = split
        task_name = self.BABI_NAMES[task]
        fact_path = join_path(data_source_path, f"babi/en-10k/{task_name}_{split}.txt")
        self.fact_dataset = get_dataset_df(fact_path, max_n_facts=max_n_facts)
...
class BabilongDataset(Dataset):
    ...
    @classmethod
    def create(cls, path, task, tokenizer, max_context_len, min_context_len=-1, seed=42, noise_data_path="pg19/", split='train'):

        fact_dataset = TaskDataset(path, task, split)  # max_n_facts=10 если нужно
        # task_name = BabilongDataset.BABI_NAMES[task]
        # fact_path = join_path(data_source_path, f"babilong/babi/en-10k/{task_name}_{split}.txt")
        # fact_dataset = TaskDataset(fact_path, task=task, split=split)

        noise_path = join_path(path, noise_data_path, split)
        noise_dataset = datasets.load_from_disk(noise_path)
        noise_sampler = SentenceSampler(noise_dataset, tokenizer, shuffle=True, random_seed=seed)

        qa_margin = 20  # leave space for questions and answers

        if min_context_len not in [-1, max_context_len]:
            assert max_context_len > min_context_len
            step_size = max((max_context_len - min_context_len)//10, 512) # no smaller than 512, no more steps than 10
            max_steps = (max_context_len - min_context_len) // step_size
            train_sample_size = [min_context_len + int(step_size * i) for i in range(1, max_steps+1)]

            if train_sample_size[-1] != max_context_len:
                train_sample_size.append(max_context_len)

            train_sample_size = [s - qa_margin for s in train_sample_size]

        else:
            train_sample_size = max_context_len - qa_margin

        dataset = cls(
            fact_dataset, noise_sampler,
            tokenizer,
...
class LocalSetBabilong(BabilongDataset):
    ...
    def __getitem__(self, item):
        ...
        return self._sample2task(sample)

    def _sample2task(self, sample):
        context = self.tokenizer.decode(sample['input_tokens'])
        info = self.get_partition_info()
        return Task(
            'qa', 'synth', len(sample['input_tokens']),
            context=context,
            answer=sample['answer'],
            question=sample['question'],
            dataset_name='babilong',
            dataset_partition=f"{info['task']}_{info['split']}"
        )
...
if __name__ == "__main__":
    ...
    dataset = LocalSetBabilong.create(
        'data_sources/babilong', 'qa2', tokenizer, min_context_len=2000, max_context_len=32000, seed=42
    )
