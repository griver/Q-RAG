...
parser.add_argument("--planner_lora",   type=str, default="./qwen_planner_lora/final",
...
# ─────────────────────────────────────────────
#  [1/3] Load Planner (HuggingFace + LoRA)
#  关键：用完后立即释放显存，再加载 vLLM
# ─────────────────────────────────────────────
print("\n[1/3] Loading planner model (HuggingFace + LoRA) …")
...
#  释放 Planner 显存（关键步骤）
# ─────────────────────────────────────────────
print("\nFreeing planner GPU memory …")
del planner_model, planner_base_model, planner_tokenizer
torch.cuda.empty_cache()
import gc; gc.collect()
print("Planner memory freed.")

# ─────────────────────────────────────────────
#  [3/3] Load QA model (vLLM)
# ─────────────────────────────────────────────
...
for hop in range(max_hops):
    active_indices = [
        i for i, sq in enumerate(all_sub_questions) if hop < len(sq)
    ]
