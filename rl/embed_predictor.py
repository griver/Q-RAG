...
class BertPredictor(nn.Module):
    ...
    def __init__(self, bert: RobertaModel, num_hidden_layers, tokenizer, model_dim, output_size, n_output) -> None:
        ...
        # <--- ИЗМЕНЕНО: Используем 'embed_tokens' вместо 'embeddings' для Qwen-архитектуры
        # Было: self.model.embeddings.load_state_dict({k: v.clone() for k, v in bert.embeddings.state_dict().items()})
        self.model.embed_tokens.load_state_dict({k: v.clone() for k, v in bert.embed_tokens.state_dict().items()})

        # <--- ИЗМЕНЕНО: Используем 'layers' вместо 'encoder.layer' для Qwen-архитектуры
        for i in range(config.num_hidden_layers):
            # Было: self.model.encoder.layer[i].load_state_dict({k: v.clone() for k, v in bert.encoder.layer[i].state_dict().items()})
            self.model.layers[i].load_state_dict({k: v.clone() for k, v in bert.layers[i].state_dict().items()})

        self.model.train()

        # <--- ИЗМЕНЕНО: Получаем vocab_size из 'embed_tokens'
        # Было: vocab_size: int = self.model.embeddings.word_embeddings.weight.shape[0]
        vocab_size: int = self.model.embed_tokens.weight.shape[0]
    ...
    def forward(self, input_ids, attention_mask, *args, **kw):
        ...
        final_output = F.normalize(prediction, p=2, dim=1)
