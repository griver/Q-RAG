...
def get_randomized_idx(n, max_indent=1000, num_splits=5):
    ...
    # Если n слишком мало для создания `num_splits-1` уникальных точек разделения,
    # просто возвращаем обычные индексы. Для этого нужно n-2 >= num_splits-1, т.е. n >= num_splits+1
    if n < num_splits + 1:
        return list(range(n))
